...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['confirmPayment'])){
    ...
    $stmt = $conn->prepare("INSERT INTO joinlist (station_name, charger_type, rate, full_name, email, phone, status, payment_method, card_last4, upi_id, payment_date)
                            VALUES (?, ?, ?, ?, ?, ?, 'Paid', ?, ?, ?, ?)");
    $stmt->bind_param("ssdsssssss", $stationName, $chargerType, $amount, $fullName, $email, $phone, $paymentMethod, $cardLast4, $upiId, $date);
...
<title>Join Waitlist Payment</title>
<style>
body {margin:0;font-family:'Segoe UI',sans-serif;background:linear-gradient(135deg,#00b4d8,#90e0ef);display:flex;justify-content:center;align-items:flex-start;min-height:100vh;padding:20px;}
...
<h2>💳 Complete Your Join Waitlist Payment</h2>
...
<p><strong>Rate:</strong> ₹<?php echo htmlspecialchars($bookingData['rate']); ?></p>
...
<?php else: ?>
<p style="text-align:center; color:#0077b6;">No waitlist data available.</p>
<?php endif; ?>
</div>
...
<script>
<?php if($bookingData): ?>
let joinBookings = JSON.parse(localStorage.getItem("joinBookings")) || [];
joinBookings.push({
...
localStorage.setItem("joinBookings", JSON.stringify(joinBookings));
<?php endif; ?>
</script>
</body>
</html>
